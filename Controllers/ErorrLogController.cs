...
namespace MledNew2023.MVC.Controllers
{
    ...
    public class ErorrLogController : Controller
    {
        private static readonly NLog.Logger _Nlog = NLog.LogManager.GetCurrentClassLogger();
        public IConfiguration Configuration { get; }
        public ErorrLogController(IConfiguration configuration)
        {
        ...
        public IActionResult Index()
        {
            ...
            BaseService.AddUserActive(currentUserID, IpAddress, 
                new MessageResults { Id = -1, message = "Xem chức năng", Tittle = "Nhật ký lỗi hệ thống" }, ConnectionString);
        ...
        public IEnumerable<FolderModel> Get()
        {
            try
            {
                ...
                {
                    if (System.IO.File.Exists(String.Format("{0}\\Error.log", _name)))
                    {
                        //Có file ghi lỗi mới hiển thị lên
        ...
        public List<FileModel> GetContent(string pathfile)
        {
            ...
            {
                var rootFolder = Directory.GetCurrentDirectory();
                string spath = String.Format("{0}\\wwwroot\\Log\\{1}\\Error.log", rootFolder, pathfile).Replace("\\", "/");
                ...
                else
                {
                    ...
                    _s.Content = String.Format("{0}/Error.log: {1}", pathfile, "Không tìm thấy file ghi nhật ký ").Replace("\\", " /").Replace(" /", "/");
                    logList.Add(_s);
                }
            }
            catch (Exception ex)
