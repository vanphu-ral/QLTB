...
{
    [Authorize]
    public class SupplieGroupController : Controller
    {
        private readonly ILogger<SupplieGroupController> _logger;
        private static readonly NLog.Logger _Nlog = NLog.LogManager.GetCurrentClassLogger();
        public IConfiguration Configuration { get; }
        public SupplieGroupController(ILogger<SupplieGroupController> logger, IConfiguration configuration)
        {
            _logger = logger;
        ...
        public IEnumerable<SupplieGroup> Get()
        {
            string ConnectionString = String.Format("{0}", Configuration.GetConnectionString("DefaultConnection"));
            ...
            try
            {
                ...
                cmd.CommandText = "GetSupplieGroup";
                SqlDataReader sqlDataReader = cmd.ExecuteReader();
                var _deviceGroupList = new List<SupplieGroup>();
                while (sqlDataReader.Read())
                {
                    var _result = new SupplieGroup();
        ...
        public MessageResults Add(SupplieGroup _deviceGroup)
        {
            ...
            try
            {
                SqlCommand cmd = sqlConnection.CreateCommand();
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.CommandText = "AddSupplieGroup";
        ...
        [HttpPost]
        //[ValidateAntiForgeryToken]
        public MessageResults Edit(SupplieGroup _deviceGroup)
        {
            MessageResults _result = new MessageResults();
            string ConnectionString = String.Format("{0}", Configuration.GetConnectionString("DefaultConnection"));
            SqlConnection sqlConnection = new SqlConnection(ConnectionString);
            try
            {
                SqlCommand cmd = sqlConnection.CreateCommand();
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.CommandText = "EditSupplieGroup";
        ...
        // POST: SupplieGroupController/Delete/5
        [HttpGet]
        public MessageResults Distroy(int Id, string DelName)
        {
            ...
            try
            {
                SqlCommand cmd = sqlConnection.CreateCommand();
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.CommandText = "DeleteSupplieGroup";
