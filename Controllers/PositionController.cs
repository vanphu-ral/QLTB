...
namespace MledNew2023.MVC.Controllers
{
    ...
    public class PositionController : Controller
    {
        ...
        public PositionController(ILogger<PositionController> logger, IConfiguration configuration)
        {
            _logger = logger;
        ...
        public IActionResult Index()
        {
            ...
            BaseService.AddUserActive(currentUserID, IpAddress, new MessageResults { Id = -1, message = "Xem chức năng", Tittle = "Quản lý chức vụ" }, ConnectionString);
            return View();
        }
        ...
        public IEnumerable<Position> Get()
        {
            ...
            try
            {
                ...
                var _positionList = new List<Position>();
                while (sqlDataReader.Read())
                {
                    var _result = new Position();
        ...
        public MessageResults Add(Position _position)
        {
            ...
            try
            {
                ...
                cmd.Parameters.AddWithValue("@Name", (_position.Name == null ? DBNull.Value : (object)_position.Name));
                cmd.Parameters.AddWithValue("@Description", _position.Description == null ? DBNull.Value : (object)_position.Description);
                if (sqlConnection.State == ConnectionState.Closed)
                {
                    sqlConnection.Open();
                }
        ...
        [HttpPost]
        //[ValidateAntiForgeryToken]
        public MessageResults Edit(Position _position)
        {
            MessageResults _result = new MessageResults();
            string ConnectionString = String.Format("{0}", Configuration.GetConnectionString("DefaultConnection"));
            SqlConnection sqlConnection = new SqlConnection(ConnectionString);
            try
            {
                SqlCommand cmd = sqlConnection.CreateCommand();
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.CommandText = "EditPosition";

                cmd.Parameters.AddWithValue("@Id", _position.Id);
                cmd.Parameters.AddWithValue("@Name", (_position.Name == null ? DBNull.Value : (object)_position.Name));
                cmd.Parameters.AddWithValue("@Description", _position.Description == null ? DBNull.Value : (object)_position.Description);
                if (sqlConnection.State == ConnectionState.Closed)
                {
